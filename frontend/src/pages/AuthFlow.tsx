  ...
  const handle = async () => {
    ...
    try {
      if (tab === 'register') {
        ...
        if (!username || !pass) { setError('Username dan password wajib diisi!'); setLoading(false); return }
        const { ok, data } = await login(username, pass)
  ...
  return (
    <div style={{ background:bgCard, borderRadius:16, padding:28, border:'1px solid #333' }}>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16 }}>
        <button onClick={loginWithGitHub} style={{ flex:1, padding:'10px', borderRadius:10, background:'#fff', border:'none', cursor:'pointer', fontWeight:600, fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
        ...
        <button style={{ flex:1, padding:'10px', borderRadius:10, background:'#fff', border:'none', cursor:'pointer', fontWeight:600, fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
      ...
      <input style={{...inputStyle, marginBottom:10}} placeholder={tab==='login' ? 'Username' : 'Email'} value={tab==='login' ? username : email} onChange={e => tab==='login' ? setUsername(e.target.value) : setEmail(e.target.value)} />
      {tab === 'register' && (
