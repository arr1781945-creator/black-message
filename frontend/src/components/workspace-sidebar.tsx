...
const workspaces = [
  { id: 1, name: "BlackMess", initial: "B", color: "bg-primary" },
]

export function WorkspaceSidebar({ activePage, onPageChange }: { activePage: string, onPageChange: (page: string) => void }) {
  const [activeWorkspace, setActiveWorkspace] = useState(1)

  return (
    <div className="flex flex-col w-[70px] bg-background border-r border-border items-center py-3 gap-2">
      {/* Workspaces */}
      ...
      <div className="flex flex-col gap-1 flex-1 pt-2">
        ...
        <NavButton icon={MessageIcon} label="Pesan" active={activePage === "dms"} onClick={() => onPageChange("dms")} />
        <NavButton icon={BellIcon} label="Aktivitas" active={activePage === "activity"} onClick={() => onPageChange("activity")} />
        <NavButton icon={SearchIcon} label="Cari" active={activePage === "search"} onClick={() => onPageChange("search")} />
      ...
      <div className="pt-2 border-t border-border">
        <button className="w-9 h-9 rounded-full bg-purple-900 flex items-center justify-center text-sm font-semibold text-white">
          J
        </button>
      </div>
    </div>
